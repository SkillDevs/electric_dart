...
Just a few things to keep in mind in the drift configuration:
* Import the **autogenerated drift schema** to use it with the drift code generation.

* Prefer to run drift on a **separate isolate** via `NativeDatabase.createInBackground` so that the UI doesn't freeze while syncing too much data. This freeze will also make the app unresponsive to Electric pings to check alive status.

* **Disable drift migrations** by providing an empty `onCreate` callback. This prevents drift from creating the tables in the local database, as this is done automatically by Electric.
...
class AppDatabase extends _$AppDatabase {
  ...
  // [IMPORTANT CHANGE!!!]
  // Provide an empty onCreate callback
  @override
...
// From the drift "Getting started": https://drift.simonbinder.eu/docs/getting-started/
LazyDatabase _openConnection() {
  ...
  return LazyDatabase(() async {
    ...
    // [IMPORTANT LINE!!!] Use `createInBackground` to run drift on a separate isolate
