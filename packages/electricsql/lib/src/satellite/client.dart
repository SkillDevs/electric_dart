...
class SatelliteClient implements Client {
  ...
  Future<void> handleIncoming(Uint8List data) async {
    try {
      print(
          '${DateTime.now()} INCOMING PROTOBUF SIZE: ${data.length.toHumanReadableSize()}');
      final sw = Stopwatch()..start();
      final messageInfo = toMessage(data);
      // if (messageInfo.msgType == SatMsgType.opLog) {
      //   print("force locking the thread by sleeping");
      //   sleep(Duration(seconds: 13));
      // }
      print('DECODE MESSAGE elapsed: ${sw.elapsed}');
...
extension SizeExtensions on num {
  ...
  String toHumanReadableSize({int round = 2, bool useBase1024 = true}) {
